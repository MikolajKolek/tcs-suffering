...
\begin{greyframe}
	...
	\begin{enumerate}
		\item Dla każdego \( \mathbb{P} \ni p < B \):
		      \begin{enumerate}
			      \item Jeśli \( N \) nie jest resztą kwadratową modulo \( p \), to pomiń to \(p\).
			      \item W przeciwnym przypadku, algorytmem Tonellego-Shanksa znajdź rozwiązanie \( x = \pm k \) równania \( x^2 = N \pmod{p} \).
			      \item Podziel przez \( p \) wszystkie \( Y[i] \), dla których \( X[i] = \pm k \pmod{p} \).
		      \end{enumerate}
		\item Jeśli \( Y[i] \) zmaleje do 1, to początkowo musiało być liczbą \( B \)-gładką.
	\end{enumerate}
\end{greyframe}
W kroku 1. (a) odrzucamy \(p\), ponieważ w takim przypadku \(X[i]^2 - N \pmod{p}\) nigdy nie może być równe 0, a więc \(p\) nie może dzielić żadnego \(Y[i]\). 
