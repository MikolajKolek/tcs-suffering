...
Rozważmy teraz wariant standardowego eksperymentu z kulami i urnami. Rzucamy \(n\) kul sekwencyjnie do \(n\) urn i dla każdej kuli symulujemy dwa rzuty. Kula trafia do tej urny z dwóch wylosowanych która jest mniej wypełniona, a remisy rozsrzygamy dowolnie. Okazuje się, że znacząco zmienia to rozkład \(\max(X_1, \dots, X_n)\), co pokazuje następujące twierdzenie
...
\begin{lemma}
	\label{binary-random-variable-binomial}
	Niech \(X_1, \dots, X_n\) to zmienne losowe, \(Y_1, \dots, Y_n\) to binarne zmienne losowe, \(Y_i = f_i(X_1, \dots, X_i)\) (jest zależne wyłącznie od zmiennych \(X_1, \dots, X_i\)) oraz \(Z \sim \Bin(n, p)\) jest niezależna od poprzednich zmiennych. Jeśli dla każdego \(i \in [n]\) oraz \((x_1, \dots, x_{i - 1})\) takiego, że \(\prob((X_1, \dots, X_{i - 1}) = (x_1, \dots, x_{i - 1})) > 0\) zachodzi
	\[
