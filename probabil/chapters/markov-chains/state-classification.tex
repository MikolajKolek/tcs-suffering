...
%<*probabil-egzamin-9-nieprzywiedlnosc-1>
\begin{definition}
	Stan \(j\) jest \textbf{osiągalny} ze stanu \(i\) jeśli istnieje \(n \geq 0\) takie, że
	\(p_{ij}(n) > 0\). Zapisujemy \(i \rightarrow j\).
\end{definition}
...
%<*probabil-egzamin-9-nieprzywiedlnosc-2>
\begin{definition}
...
\begin{definition}
	...
	Stan lub łańcuch, który nie jest okresowy nazywamy \textbf{nieokresowym}.
\end{definition}
%</probabil-egzamin-9-nieprzywiedlnosc-2>

%<*probabil-egzamin-9-okres-nieprzywiedlnego>
\begin{lemma}
...
\begin{proof}
	...
	\[
		m+n+l \in \mathcal{T}(i) \implies o(i) \mid m+n+l
	\]
	\[
		m+l \in \mathcal{T}(i) \implies o(i) \mid m+l
	\]
	\[
		\implies o(i) \mid n \implies o(i) \leq o(j)
	\]
	Analogicznie dowodzimy w drugą stronę, otrzymując równość.
\end{proof}
%</probabil-egzamin-9-okres-nieprzywiedlnego>

%<*probabil-egzamin-9-wlasnosc-nieprzywiedlnego-nieokresowego>
\begin{lemma}
...
\begin{proof}
	...
	\]
	Niech \( i, j \in S \). Z lematu Schura mamy:
	\[
...
%<*probabil-egzamin-9-stany-powracajace-chwilowe>
...
\begin{definition}
	...
	jako
	\[
		f_{ij}(n) = \prob(X_n = j \land X_{n-1} \neq j \dots X_{1} \neq j \mid X_0 = i) = \prob(T_j^+ = n \mid X_0 = i)
	\]
\end{definition}
...
\begin{definition}
	...
	jako
	\[
		f_{ij} = \sum_{n=1}^{\infty} f_{ij}(n) = \prob(T_j^+ < \infty \mid X_0 = i)
	\]
\end{definition}
%</probabil-2025-11-19-stany-definicje2>
...
\begin{definition}
	...
	dodatkowo
	\[
		\ev{T_{i, j}} = \ev{T_j^+ \mid X_0 = i} = \sum_{n = 1}^\infty n \prob(T_j^+ = n \mid X_0 = i) = \sum_{n = 1}^\infty n f_{ij}(n)
	\]
\end{definition}
...
\begin{example}
	...
	\]
	\[
		\ev{T_{1, 1}} = \sum_{t = 1}^\infty t \prob(T_i^+ = t \mid X_0 = 1) = \sum_{t = 1}^\infty t \pars{\frac{1}{2} \dots \frac{t-1}{t} \frac{1}{t+1}} = \sum_{t = 1}^\infty \frac{1}{t+1} \to \infty
	\]
	Z \(f_{1, 1} = 1\) stan 1 jest powracający, a z \(\ev{T_{1, 1}} \to \infty\) jest zerowy.
\end{example}

%<*probabil-egzamin-9-wlasnosc-nieprzywiedlnego-skonczonego>
\begin{theorem}
	W skończonym, nieprzywiedlnym łańcuchu Markowa zachodzi
	\[
		\forall_{x, y \in S} \, \ev{T_{x, y}} < \infty
	\]
\end{theorem}
\begin{proof}
	Nieprzywiedlność oraz skończoność dają nam
	\[
		\exists_{r > 0, \varepsilon > 0} \, \forall_{x, y \in S} \, \exists_{j \in [r]} \, p_{x, y}(j) > \varepsilon
	\]
	Mimo tego, że linia ta może początkowo być trudna do przetworzenia, jest całkiem prosta. Nieprzywiedlność mówi nam że dla każdego \((x, y)\), \(y\) jest osiągalne z \(x\), a więc \(\exists_n \, p_{x, y}(n) > 0\). Nasze \(r\) to po prostu maksimum po tych \(n\) dla wszystkich par \((x, y)\), a \(\varepsilon\) to minimum z wartości \(p_{x, y}(n)\).
	
	Następnie chcemy pokazać, że
	\[
		\prob(T_y^+ > kr \mid X_0 = x) \leq \prob(T_y^+ > (k - 1)r \mid X_0 = x)(1 - \varepsilon)
	\]
	Dlaczego tak jest? Otóż wiemy, że dla każdej możliwej wartości \(z = X_{(k-1)r}\) zachodzi 
	\[
		\exists_{j \in [r]} \, p_{z, y}((k - 1)r + j) > \varepsilon
	\]
	a więc z prawdopodobieństwem przynajmniej \(\varepsilon\) odwiedzimy \(y\) w następnych \(r\) krokach. W takim razie, jeśli \(T_y^+ > (k-1)r\), to \(\prob(T_y^+ \leq kr) \geq \varepsilon\), a więc prawdopodobieństwo tego, że nie dojdziemy do \(y\) jest ograniczone od góry przez \(1 - \varepsilon\), co daje nam naszą nierówność.
	
	Następnie, poprzez prostą indukcję można pokazać, że
	\[
		\prob(T_y^+ > kr \mid X_0 = x) \leq (1 - \varepsilon)^k
	\]
	Teraz, przechodząc do finalnego dowodu i korzystając z \ref{expected-value-of-natural-random-variable}
	\begin{align*}
		\ev{T_{x, y}} &= \ev{T_y^+ \mid X_0 = x}\\
		&= \sum_{t = 0}^\infty \prob(T_y^+ > t \mid X_0 = x)\\
